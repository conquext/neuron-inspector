  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        neuron_url: { type: "string", description: "Neuron API base URL (default: https://api.neuron.ng). Set this if self-hosting Neuron." },
      },
...
async function approveViaWhatsapp(
  ...
  const baseUrl = (args.neuron_url as string) ?? "https://api.neuron.ng";
    ...
    while (Date.now() - startTime < timeoutSec * 1000) {
      ...
      try {
        const pollResponse = await fetch(
          `${baseUrl}/api/v1/approvals/${approvalId}`,
          {
